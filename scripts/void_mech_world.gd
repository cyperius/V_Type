...
func _physics_process(delta: float) -> void:
	for girder in girders: # Bewegung der Girders; Werterange wird jeweils in set_girders_stats_range
		...
		if girder.global_position.x <= -200 or girder.global_position.y < -200 or girder.position.y > 2600: 
			...
			girder.global_position.x = 4300
			girder.global_position.y = randi_range(100, 2400)
